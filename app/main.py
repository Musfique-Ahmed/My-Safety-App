...
from app.core.config import BASE_DIR, CONTENTS_DIR, STATIC_DIR, UPLOADS_DIR
...
from app.db import fetch_one, fetch_all, execute, insert_and_get_id, parse_json_field as parse_json_value
...
# Mount static files — paths are anchored to BASE_DIR so the package can be
# launched from anywhere (uvicorn, gunicorn, pytest, etc.).
...
app.mount("/contents", StaticFiles(directory=str(CONTENTS_DIR)), name="contents")
...
# ==================== PYDANTIC MODELS ====================
# All request/response schemas live under app.schemas (split by domain) and
# are re-exported here for handlers that import from this module. The local
# classes that used to live here have been removed in favour of the package
# layout — see app.schemas for the canonical definitions.
from app.schemas import (  # noqa: F401  (re-exports for in-module use)
    AdminCrimeCreate,
    CaseAssignment,
    ChatMessage,
    CrimeData,
...
# main_admin.py, now app/admin_main.py) under /admin-api so the admin dashboard
# can still call them while everything shares one uvicorn process on port 8000.
try:
    from app.admin_main import app as admin_subapp  # noqa: E402
    app.mount("/admin-api", admin_subapp)
except Exception as exc:  # pragma: no cover - dev convenience
    logging.warning("Could not mount admin_main under /admin-api: %s", exc)
